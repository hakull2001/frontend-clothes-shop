...
class _UserService {
  ...
  public payment(amount : String){    
    return HttpService.post(`/users/payment`,{
      body: {vnpAmount : amount},
    }).pipe(map<any, String>((response)=>{
  ...
  public checkPayment(value : String){
    return HttpService.get(`/users/check/${value}`, {
    }).pipe(
      map<any, User>((response) => {
        console.log(response);
